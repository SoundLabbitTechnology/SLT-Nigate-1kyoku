...
フロントは静的配信、ゲーム進行は `api/index.ts` の Express + WebSocket です。Hobby プランでは接続が最大約5分で切れるので、クライアントが自動再接続します。

ルーム状態はプロセスメモリです。同時に複数インスタンスが立つと、同じコードでも別ルームになります。イベント利用で参加者同士が見えない場合は、Vercel の Storage で Redis を足すか、Cloud Run など常駐サーバへ移してください。
